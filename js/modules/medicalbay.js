...
import { voiceAcknowledgeEnergy, voiceAcknowledgeMood, voiceAcknowledgePain, voicePhrases, voiceSpeak } from "./voice.js";
...
export async function resetMedicalBayForm() {
    ...
    voiceSpeak(voicePhrases().logCleared);
}
...
export function saveMedicalBayLog() {
    ...
    voiceSpeak("Medical Bay entry recorded, Captain.");

    const painLevel = Number(data.overallPain);
    const energyLevel = Number(data.energy);
    const moodLevel = Number(data.mood);

    if (painLevel >= 7) {
        voiceAcknowledgePain(painLevel);
    } else if (energyLevel <= 3) {
        voiceAcknowledgeEnergy(energyLevel);
    } else if (moodLevel <= 3) {
        voiceAcknowledgeMood(moodLevel);
    }
}
